// ==================== CONFIG =====================
const YOUR_API_KEYS = ["ly"];
const RAPIDAPI_HOST = "youtube-media-downloader.p.rapidapi.com";
const RAPIDAPI_KEY = "68dd515dd3mshe757fcf6151ac56p152e78jsn96ec259df288";
const CACHE_TIME = 3600 * 1000;
// =================================================

const cache = new Map();

function cleanOxmzoo(value) {
    if (typeof value == "string") {
        return value.replace(/@oxmzoo/gi, "").trim();
    }
    if (Array.isArray(value)) {
        return value.map(cleanOxmzoo);
    }
    if (value && typeof value === "object") {
        const cleaned = {};
        for (const key of Object.keys(value)) {
            if (key.toLowerCase().includes("oxmzoo")) continue;
            cleaned[key] = cleanOxmzoo(value[key]);
        }
        return cleaned;
    }
    return value;
}

export default async function handler(req, res) {
    res.setHeader("Access-Control-Allow-Origin", "*");
    
    // Sirf GET allow
    if (req.method !== "GET") {
        res.setHeader("Content-Type", "application/json; charset=utf-8");
        return res.status(405).json({ error: "method not allowed" });
    }

    const { videoId, key } = req.query || {};

    // Param check
    if (!videoId || !key) {
        res.setHeader("Content-Type", "application/json; charset=utf-8");
        return res.status(400).json({ 
            error: "missing parameters", 
            details: "Use: ?videoId=YouTubeVideoID&key=SPLEXXO" 
        });
    }

    const cleanVideoId = String(videoId).trim();
    const cleanKey = String(key).trim();

    // API key check
    if (!YOUR_API_KEYS.includes(cleanKey)) {
        res.setHeader("Content-Type", "application/json; charset=utf-8");
        return res.status(403).json({ error: "invalid key" });
    }

    // Cache check
    const now = Date.now();
    const cached = cache.get(cleanVideoId);

    if (cached && now - cached.timestamp < CACHE_TIME) {
        res.setHeader("X-Proxy-Cache", "HIT");
        res.setHeader("Content-Type", "application/json; charset=utf-8");
        return res.status(200).send(cached.response);
    }

    // YouTube Media API call
    const url = `https://${RAPIDAPI_HOST}/v2/video/details?videoId=${encodeURIComponent(cleanVideoId)}&urlAccess=normal&videos=auto&audios=auto`;

    try {
        const upstream = await fetch(url, {
            headers: {
                'x-rapidapi-host': RAPIDAPI_HOST,
                'x-rapidapi-key': RAPIDAPI_KEY
            }
        });

        const raw = await upstream.text();

        if (!upstream.ok || !raw) {
            res.setHeader("Content-Type", "application/json; charset=utf-8");
            return res.status(502).json({
                error: "YouTube API failed",
                details: `HTTP ${upstream.status}`,
            });
        }

        let responseBody;

        try {
            // JSON try parse
            let data = JSON.parse(raw);

            // @oxmzoo clean
            data = cleanOxmzoo(data);

            // Apna clean branding
            data.developer = "TG : @PYPHANTOM";
            data.credit_by = "channel : @naexh";
            data.powered_by = "shaurya shukla";

            responseBody = JSON.stringify(data);
        } catch (e) {
            // Agar JSON nahi hai, to raw text se @oxmzoo hata do
            const cleanedText = raw.replace(/@oxmzoo/gi, "").trim();
            responseBody = cleanedText;
        }

        // Cache save
        cache.set(cleanVideoId, {
            timestamp: Date.now(),
            response: responseBody,
        });

        res.setHeader("X-Proxy-Cache", "MISS");
        res.setHeader("Content-Type", "application/json; charset=utf-8");
        return res.status(200).send(responseBody);
    } catch (err) {
        res.setHeader("Content-Type", "application/json; charset=utf-8");
        return res.status(502).json({
            error: "YouTube API request error",
            details: err.message || "unknown error",
        });
    }
      }
